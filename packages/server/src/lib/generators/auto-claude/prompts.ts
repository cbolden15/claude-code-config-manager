...
export function generateAutoClaudePrompts(options: PromptsOptions): GeneratedPromptFile[] {
  ...
  const { result } = timeOperationSync('prompts generation', () => {
    ...
    generatedFiles.length = prompts.length;

    // Process prompts in batch for better performance with large arrays
    const batchSize = prompts.length > 20 ? 10 : prompts.length;
    ...
    return generatedFiles.filter(Boolean); // Remove any undefined entries
  }, prompts.length);
...
const injectionPatterns = {
  specDirectory: /\{\{specDirectory\}\}/g,
  projectContext: /\{\{projectContext\}\}/g,
  mcpDocumentation: /\{\{mcpDocumentation\}\}/g,
  unresolved: /\{\{[^}]+\}\}/g
};

// Cache for header comment to avoid string concatenation on each call
const headerComment = '<!-- Generated by Claude Code Config Manager -->\n' +
                      '<!-- Do not edit manually - sync from CCM to update -->\n\n';

/**
 * Processes prompt content by replacing injection points with actual values - optimized version
 */
function processPromptContent(
  ...
  // Early return if no injection points are configured
  if (!prompt.injectionPoints) {
    return addHeaderComment(content);
  }

  // Use more efficient approach for injection point replacement
  // Reset global regex lastIndex to ensure correct matching
  injectionPatterns.specDirectory.lastIndex = 0;
  injectionPatterns.projectContext.lastIndex = 0;
  injectionPatterns.mcpDocumentation.lastIndex = 0;

  // Replace injection points using cached regex patterns
  if (prompt.injectionPoints.specDirectory && injectionContext.specDirectory) {
    content = content.replace(injectionPatterns.specDirectory, injectionContext.specDirectory);
  }

  if (prompt.injectionPoints.projectContext && injectionContext.projectContext) {
    content = content.replace(injectionPatterns.projectContext, injectionContext.projectContext);
  }

  if (prompt.injectionPoints.mcpDocumentation && injectionContext.mcpDocumentation) {
    content = content.replace(injectionPatterns.mcpDocumentation, injectionContext.mcpDocumentation);
  }

  // Remove any remaining unresolved injection points
  injectionPatterns.unresolved.lastIndex = 0;
  content = content.replace(injectionPatterns.unresolved, '<!-- Injection point not resolved -->');

  return addHeaderComment(content);
}

/**
 * Add header comment to content if not already present - optimized function
 */
function addHeaderComment(content: string): string {
  // Quick check to avoid processing if header already exists
  if (content.includes('Generated by Claude Code Config Manager')) {
    return content;
  }

  // More efficient front matter handling with single pass
  if (content.startsWith('---\n')) {
    const frontMatterEnd = content.indexOf('\n---\n', 4);
    if (frontMatterEnd !== -1) {
      const frontMatter = content.substring(0, frontMatterEnd + 5);
      const restContent = content.substring(frontMatterEnd + 5);
      return frontMatter + headerComment + restContent;
    }
  }
