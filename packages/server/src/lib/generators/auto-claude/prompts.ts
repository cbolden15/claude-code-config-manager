...
function processPromptContent(
  ...
  if (prompt.injectionPoints) {
    // Batch replace injection points for better performance
    const replacements = [];

    if (prompt.injectionPoints.specDirectory && injectionContext.specDirectory) {
      replacements.push([/\{\{specDirectory\}\}/g, injectionContext.specDirectory]);
    }

    if (prompt.injectionPoints.projectContext && injectionContext.projectContext) {
      replacements.push([/\{\{projectContext\}\}/g, injectionContext.projectContext]);
    }

    if (prompt.injectionPoints.mcpDocumentation && injectionContext.mcpDocumentation) {
      replacements.push([/\{\{mcpDocumentation\}\}/g, injectionContext.mcpDocumentation]);
    }
  ...
  // Check if we need to add header comment - optimized check
